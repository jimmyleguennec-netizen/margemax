...
with st.expander("🔧 Diagnostic réseau (temporaire)", expanded=False):
    st.code(f"SUPABASE_URL = {SUPABASE_URL!r}")
    col_a, col_b = st.columns(2)
    with col_a:
        if st.button("Tester Supabase"):
            try:
                r = requests.get(SUPABASE_URL, timeout=8)
                st.success(f"Supabase OK — HTTP {r.status_code}")
            except Exception as e:
                st.error(f"Supabase échoue : {e!r}")
    with col_b:
        if st.button("Tester un autre site (example.com)"):
            try:
                r = requests.get("https://example.com", timeout=8)
                st.success(f"example.com OK — HTTP {r.status_code}")
            except Exception as e:
                st.error(f"example.com échoue aussi : {e!r}")
...
def get_supabase_client() -> Client:
    """Client anonyme partagé — utilisé uniquement pour les opérations
    publiques (connexion, inscription). Ne jamais y attacher un jeton
    utilisateur : il est mis en cache et partagé entre TOUTES les
    sessions de l'app."""
    return create_client(SUPABASE_URL, SUPABASE_KEY)


def get_authenticated_client(access_token: str) -> Client:
    """Client dédié à une seule requête, avec le jeton de l'utilisateur
    attaché, pour que les policies RLS (auth.uid() = user_id) fonctionnent.
    Jamais mis en cache — chaque appel crée une instance isolée, propre
    à cette requête, pour ne jamais mélanger les sessions utilisateur."""
...
def _client_for(access_token: str | None) -> Client:
    return get_authenticated_client(access_token) if access_token else supabase
...
def ensure_profile_exists(user_id: str, email: str, access_token: str | None = None) -> dict:
    ...
    try:
        _client_for(access_token).table("profiles").insert(new_profile).execute()
    except Exception:
        pass
...
def increment_search_count() -> None:
    ...
    if user:
        try:
            _client_for(user.get("access_token")).table("profiles").update(
